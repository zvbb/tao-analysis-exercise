...
% 令$h : [a, b] \to [-\phi(a), -\phi(b)], h(x) := -\phi(x)$，
% 那么$h$是单调递增的可微函数。

% $f: [\phi(b), \phi(a)] \to \mathbb{R}$是$[\phi(b), \phi(a)]$上的
% 黎曼可积的函数，由习题11.10.3可知，
% \begin{align*}
%   \int_{[-\phi(a), -\phi(b)]} g = \int_{[\phi(b), \phi(a)]} f
% \end{align*}
% 其中$g(x) := f(-x)$，可得$g$是$[-\phi(a), -\phi(b)]$上的黎曼可积的函数。

% 利用命题11.10.7可得
% \begin{align*}
%   \int_{[a, b]} (f \circ h) h^\prime & = \int_{[-\phi(a), -\phi(b)]} g \\
%                                      & = \int_{[\phi(b), \phi(a)]} f
% \end{align*}
